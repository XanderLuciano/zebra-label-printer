...
- **`printerSelection`** on every response carries `reason` (from
  `PRINTER_SELECTION_REASONS` in `constants.ts`, imported by `openapi.ts` so the documented
  set can't drift), `printerName` for display, and a plain-language `message`. The message
  exists because a `reason` code alone left an integrator asking what it meant.
  `PRINTER_STOCK_MISMATCH` warns when a pinned `labelSize` disagrees with the target
  printer's stock — only reachable via a pin, since otherwise the geometry comes from the
  printer.
...
|---------|------|---------|
| v0.8.0 | 2026-09-04 | **`printerSelection` explains itself.** Adds `printerName` for display and a plain-language `message` saying how the printer was chosen and what to change. **Breaking (response only):** the `reason` values `explicit` and `pinned-label-size` are renamed `explicit-printer` and `explicit-label-size`, so the set reads as parallel — the old name left the first integrator asking what it meant. New `PRINTER_STOCK_MISMATCH` warning for a pinned `labelSize` that disagrees with the target printer's stock, which was the one remaining way to get a cropped label with nothing in the response saying so. `PRINTER_SELECTION_REASONS` now lives in `constants.ts` and is imported by `openapi.ts`, so the documented set cannot drift from the sendable one |
