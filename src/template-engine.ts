/**
 * Template engine — the heart of the label designer, and of template printing.
 *
 ...
 *
 * Lives in `src/`, not the web app, because both the designer and
 * `POST /api/print/template/{shortName}` resolve templates. Two copies would mean
 * the same template printing differently depending on whether a person or a
 * webhook asked, with the preview agreeing with only one.
 * `web/app/composables/useTemplateEngine.ts` re-exports this.
 *
 * So it is compiled twice — CommonJS by `tsc`, ESM by Vite. Keep it free of Node
 * built-ins, browser globals, and anything needing a bundler. The SVG `transform`
 * strings are the one frontend-smelling part; they stay because they derive from
 * the same rotation geometry the printed output uses, and splitting them out would
 * put that geometry in two places again.
 */
 ...
 * Selectable `^A` fonts.
 *
 * Re-exported from ./zpl-fonts, which owns the per-font metrics, so there is one
 * list to keep in step instead of two.
 */
export { ZPL_FONT_IDS as ZPL_FONTS } from './zpl-fonts'
